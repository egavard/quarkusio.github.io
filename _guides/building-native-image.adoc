...
FROM quay.io/quarkus/centos-quarkus-maven:{graalvm-version}-javaXX AS build
...
[WARNING]
====
You have to specify the correct java version to use. You can visualize available version of the image https://quay.io/repository/quarkus/centos-quarkus-maven?tag=latest&tab=tags[in the quay.io repository]
====
